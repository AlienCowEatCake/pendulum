...
CScene2D::CScene2D(bool haveNegativeY, QWidget* parent)
...
void CScene2D::adjustAxis(float & min, float & max, int & numTicks)
{
...
void CScene2D::resize(float x0, float x1, float y0, float y1)
{
    m_minX = x0;
    m_minY = y0;
    m_maxX = x1;
...
QPoint CScene2D::toWindow(float x, float y) const
{
...
void CScene2D::drawGraph(QPaintDevice * device)
{
    ...
    if(m_haveNegativeY)
    {
        ...
        painter.drawText(toWindow(-0.05f, 0.955f), m_labelY);
    }
    else
    {
...
QColor CScene2D::gridSmallColor()
{
