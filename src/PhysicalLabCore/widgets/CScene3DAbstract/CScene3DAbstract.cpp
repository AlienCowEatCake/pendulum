...
void CScene3DAbstract::setDefaultScene(const SceneDefault & sceneDefault)
{
...
void CScene3DAbstract::setLightParameters(const LightParameters & lightParameters)
{
...
void CScene3DAbstract::resizeGL(int nWidth, int nHeight)
{
...
void CScene3DAbstract::mousePressEvent(QMouseEvent* pe)
{
    m_mousePosition = pe->pos();
}

/// @todo Зачем это нужно?
void CScene3DAbstract::mouseReleaseEvent(QMouseEvent*)
{
    updateGL();
}

void CScene3DAbstract::mouseMoveEvent(QMouseEvent* pe)
{    
...
void CScene3DAbstract::wheelEvent(QWheelEvent* pe)
{
    if((pe->delta())>0)
        scale_plus();
    else if((pe->delta())<0)
...
void CScene3DAbstract::keyPressEvent(QKeyEvent* pe)
{
    switch (pe->key())
    {
    case Qt::Key_Plus:
...
void CScene3DAbstract::scale_plus()
{
...
void CScene3DAbstract::scale_minus()
{
...
void CScene3DAbstract::rotate_up()
{
    m_sceneParameters.xRot += 1.0f;
}

void CScene3DAbstract::rotate_down()
{
    m_sceneParameters.xRot -= 1.0f;
}

void CScene3DAbstract::rotate_left()
{
    m_sceneParameters.zRot += 1.0f;
}

void CScene3DAbstract::rotate_right()
{
    m_sceneParameters.zRot -= 1.0f;
}

void CScene3DAbstract::translate_down()
{
    m_sceneParameters.zTra -= 0.05f;
}

void CScene3DAbstract::translate_up()
{
    m_sceneParameters.zTra += 0.05f;
}

/// @brief Сброс сцены в первоначальное состояние
void CScene3DAbstract::defaultScene()
{
...
const GLfloat * CScene3DAbstract::lightAmbient(GLenum light_ID) const
{
...
const GLfloat * CScene3DAbstract::lightDiffuse(GLenum light_ID) const
{
    for(QList<LightParameters>::ConstIterator it = m_lightParameters.begin(); it != m_lightParameters.end(); ++it)
    {
        if(it->lightID == light_ID)
...
const GLfloat * CScene3DAbstract::lightPosition(GLenum light_ID) const
{
    for(QList<LightParameters>::ConstIterator it = m_lightParameters.begin(); it != m_lightParameters.end(); ++it)
    {
        if(it->lightID == light_ID)
...
void CScene3DAbstract::updateLight()
{
    for(QList<LightParameters>::ConstIterator it = m_lightParameters.begin(); it != m_lightParameters.end(); ++it)
    {
